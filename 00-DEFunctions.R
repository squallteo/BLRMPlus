...
###################################
###################################
#functions for methods based on UPM
# checkstop_UPM = function(probdt, Pint, target.upm = 0.5, min.subj.MTD = 6, max.subj = 40, ewoc = 0.25){
#   browser()
#   upmdt <- probdt[c("Punder", "Ptarget", "Pover")] / diff(Pint) %x% t(rep(1, length(DoseProv)))
#   
#   probdt <- probdt %>% mutate(Toxic = (Pover > ewoc)*1)
#   Ntotal <- probdt %>% summarize_at("Npat",sum) %>% select(Npat)
#   currdt <- probdt %>% filter(Current == 1)
